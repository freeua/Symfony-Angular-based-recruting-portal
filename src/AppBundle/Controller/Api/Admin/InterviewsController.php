...
class InterviewsController extends FOSRestController
{
    ...
    public function getInterviewsAction(Request $request){
        ...
        $interviews = $em->getRepository("AppBundle:Interviews")->getAllInterviews([1,2,3], $request->query->all());
        $applicants = $em->getRepository("AppBundle:Applicants")->getAllApplicants([1,2], $request->query->all());
        ...
        if($request->query->getBoolean('csv', false) == false){
            ...
            $view = $this->view([
                'items'=>$this->transformDetailsAll($pagination->getItems(), $em),
                'pagination' => [
            ...
            $result = [];
            $interviewsData = $this->transformDetailsAll($interviews, $em);
            if(!empty($interviewsData)){
                foreach ($interviewsData as $interview){
    ...
    public function getAwaitingApplicantsAction(Request $request){
        $em = $this->getDoctrine()->getManager();
        $awaitingApplicants = $em->getRepository("AppBundle:Applicants")->findBy(
            ['status'=>1],
            ['created'=>'DESC']
        );
        ...
        $view = $this->view([
            'items'=>$this->transformDetailsApplicants($pagination->getItems(), $em),
            'pagination' => [
                'current_page_number' => $pagination->getCurrentPageNumber(),
                'total_count' => $pagination->getTotalItemCount(),
            ]
    ...
    public function getSetUpInterviewsCandidateAction(Request $request){
        $em = $this->getDoctrine()->getManager();
        $setUpInterviews = $em->getRepository("AppBundle:Interviews")->findBy(
            ['status'=>1, 'type'=>true],
            ['created'=>'DESC']
        );
    ...
    public function getPendingInterviewAction(Request $request){
        $em = $this->getDoctrine()->getManager();
        $pendingInterviews = $em->getRepository("AppBundle:Interviews")->findBy(
            ['status'=>2],
            ['created'=>'DESC']
        );
    ...
    public function getPlacedInterviewAction(Request $request){
        $em = $this->getDoctrine()->getManager();
        $placedInterviews = $em->getRepository("AppBundle:Interviews")->findBy(
            ['status'=>3],
            ['created'=>'DESC']
        );
