...
class LoggingController extends FOSRestController
{
    ...
    public function getAllLoggingAction(Request $request){
        ...
        $logging = $em->getRepository("AppBundle:Logging")->getAllLogging($request->query->all());
        ...
        $view = $this->view([
            'items'=>$this->generateDataLogging($pagination->getItems()),
            'pagination' => [
     ...
     * @return array
     */
    private function generateDataLogging(array $items){
        $logging = [];
        $actions = [
            'Create Candidate',
            'Edit Candidate',
