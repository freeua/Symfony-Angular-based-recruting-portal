...
class JobRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function getClientJobsWithCriteria($id, $status = false){
        $query = $this->createQueryBuilder('j')
            ->select('j.id, j.jobTitle, j.articlesFirm, j.gender, j.ethnicity, j.qualification, j.nationality, j.availability, j.location, j.postArticles, j.salaryRange')
            ->where("j.user = :id")
            ->setParameter('id', $id);
    ...
    public function getClientJobById($userID, $jobID){
        return $this->createQueryBuilder('j')
            ->select('j.id, j.jobTitle, j.industry, j.companyName, j.companyAddress, j.addressCountry, j.addressState, j.addressZipCode,'.
                ...
                'j.location, j.postArticles, j.salaryRange, j.approve, j.status, j.created as createdDate,'.
                '(Select COUNT(a.id) FROM AppBundle:Applicants as a WHERE a.job = j.id AND a.status = 1) as awaitingCount,'.
                '(Select COUNT(a1.id) FROM AppBundle:Applicants as a1 WHERE a1.job = j.id AND a1.status = 2) as shortListCount,'.
                '(Select COUNT(a2.id) FROM AppBundle:Applicants as a2 WHERE a2.job = j.id AND a2.status = 3) as approvedCount'
            )
    ...
    public function getAllJob($params=array()){
        $query = $this->createQueryBuilder('j')
            ...
            ->select('j.id, j.companyName, j.jobTitle, j.created as jobDate, u.firstName, u.lastName, u.email, u.phone, j.approve, j.status')
            ->where("j.user = u.id")
    ...
    public function getJobApprove($params=array()){
        $query = $this->createQueryBuilder('j')
            ->from("AppBundle:User", "u")
            ->select('j.id, j.companyName, j.jobTitle, j.created as jobDate, u.firstName, u.lastName, u.email, u.phone')
            ->where("j.user = u.id")
    ...
    public function getJobById($jobID){
        return $this->createQueryBuilder('j')
            ->select('j.id, j.jobTitle, j.industry, j.companyName, j.companyAddress, j.addressCountry, j.addressState, j.addressZipCode,'.
                ...
                'j.roleDescription, j.roleDescriptionChange, j.closureDate, j.articlesFirm, j.gender, j.ethnicity, j.qualification, j.nationality,'.
                'j.availability, j.location, j.postArticles, j.salaryRange, j.approve, j.status, j.created as createdDate, j.started, j.filled, j.spec'
            )
            ->where('j.id = :jobID')
            ->setParameter('jobID', $jobID)
            ->getQuery()
    ...
    public function getJobsForCandidate(ProfileDetails $profileDetails, $params = array()){
        ...
        if(isset($params['search']) && !empty($params['search'])){
           $query->andWhere('j.jobTitle LIKE :search')
                ->setParameter('search', '%'.$params['search'].'%');
        }
