...
class SendEmail
{
    ...
    const TYPE_CANDIDATE_VIDEO_APPROVE = 'videoApproveStatus';
                    ...
                    if(method_exists($candidateNotify, $method) && method_exists($candidateNotify, $methodCheck)){
                        if($candidateNotify->$methodCheck() == true){
                            ...
                            if($delay == 1){
                                ...
                                if($candidateNotify->getNotifyWhatsApp() == true){
                                    self::sendWhatsAppNotifyCandidate($em, $candidate, $type,$emailData);
                                }
                            }
                            elseif(in_array($delay, [2,3])){
     ...
     * @param array $params
     */
    public static function sendWhatsAppNotifyCandidate(EntityManager $em, User $user, $type, $params=array()){
        if(!empty($user->getPhone())){
            ...
            $candidateNotify = $em->getRepository("AppBundle:NotificationCandidate")->findOneBy(['user'=>$user, 'notifyWhatsApp'=>true]);
            if($candidateNotify instanceof NotificationCandidate){
