...
export class BusinessService extends SettingsApiService{
   ...
   */
  public async getBusinessCandidatesCount(search, articlesFirm = null, gender  = null, qualification = null, nationality = null,
                                          ethnicity = null, video = null, location = null, availability = null, articlesCompletedStart = null, articlesCompletedEnd = null): Promise<any> {
    const headers = await this.createAuthorizationHeader();
    const params = `search=${search}&articlesFirm=${articlesFirm}&gender=${gender}&ethnicity=${ethnicity}&nationality=${nationality}&location=${location}&qualification=${qualification}&video=${video}&availability=${availability}&articlesCompletedStart=${articlesCompletedStart}&articlesCompletedEnd=${articlesCompletedEnd}`;
    return this._http.get<any>(`/api/business/candidate/count?${ params }`, headers)
        .toPromise()
