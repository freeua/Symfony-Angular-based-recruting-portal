...
export class CandidateService extends SettingsApiService {
   ...
   * @returns {Promise<any>}
   */
  public async uploadVideo(data: FormData): Promise<any> {
    const headers = await this.createAuthorizationHeader();

    return this._http.post('/api/candidate/profile/video', data, headers)
      .toPromise()
      .catch(this.handleError);
  }
   ...
   * @returns {Promise<any>}
   */
  public async uploadBlobVideo(data): Promise<any> {
    const headers = await this.createAuthorizationHeader();
