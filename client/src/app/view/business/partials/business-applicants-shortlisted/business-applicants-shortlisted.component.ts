...
@Component({
  selector: 'app-business-applicants-shortlisted',
  templateUrl: './business-applicants-shortlisted.component.html',
  styleUrls: ['./business-applicants-shortlisted.component.scss']
})
export class BusinessApplicantsShortlistedComponent implements OnInit {
  ...
  @ViewChild('jobStatus') _jobStatus: ElementRef;
  @ViewChild('filterItem') _filterItem: ElementRef;

  public applicantsShortlisted = new Array<BusinessApplicant>();
  ...
  public async fetchListOfApplicants(jobID: number, page, limit, search, articlesFirm, gender, ethnicity, nationality, location, qualification, video, availability, articlesCompletedStart, articlesCompletedEnd): Promise<void> {
    ...
    if (startDate > endDate) {
      ...
      try {
        const response = await this._businessService.getApplicantsShortlisted(data);
        ...
        response.items.forEach((item) => {
          ...
          this.applicantsShortlisted.push(item);
        });

        if (response.pagination.total_count === this.applicantsShortlisted.length) {
          this.loadMoreCheck = false;
        }
        else if (response.pagination.total_count !== this.applicantsShortlisted.length) {
  ...
  public async getApplicantsCount(jobID, page, limit, search, articlesFirm, gender, ethnicity, nationality, location, qualification, video, availability, articlesCompletedStart, articlesCompletedEnd): Promise<void> {
    ...
    if (startDate > endDate) {
      ...
      try {
        const response = await this._businessService.getApplicantsCount(2, data);
