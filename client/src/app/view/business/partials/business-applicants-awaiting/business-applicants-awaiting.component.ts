...
@Component({
  selector: 'app-business-applicants-awaiting',
  templateUrl: './business-applicants-awaiting.component.html',
  styleUrls: ['./business-applicants-awaiting.component.scss']
})
export class BusinessApplicantsAwaitingComponent implements OnInit {
  ...
  public applicantsAwaitingApproval = new Array<BusinessApplicant>();
  ...
  ngOnInit() {
    ...
    this.filterForm = new FormGroup({
      ...
      articlesCompletedStart: new FormControl(null),
      articlesCompletedEnd: new FormControl(null)
    });
  ...
  public resetArrayPagination(): void{
    this.applicantsAwaitingApproval = [];
    this.pagination = 1;
  }
   ...
   * Load pagination
   */
  public loadPagination(jobID, page, limit, search, articlesFirm, gender, ethnicity, nationality, location, qualification, video, availability, articlesCompletedStart, articlesCompletedEnd): void {
    this.pagination++;
   ...
   * @returns void
   */
  public async fetchListOfApplicants(jobID, page, limit, search, articlesFirm, gender, ethnicity, nationality, location, qualification, video, availability, articlesCompletedStart, articlesCompletedEnd): Promise<void> {
    ...
    if (startDate > endDate) {
      ...
      try {
        const response = await this._businessService.getApplicantsAwaitingApproval(data);
        ...
        response.items.forEach((item) => {
          ...
          this.applicantsAwaitingApproval.push(item);
        });

        if (response.pagination.total_count === this.applicantsAwaitingApproval.length) {
          this.loadMoreCheck = false;
        }
        else if (response.pagination.total_count !== this.applicantsAwaitingApproval.length) {
  ...
  public async searchFilter(jobID, page, limit, search, articlesFirm, gender, ethnicity, nationality, location, qualification, video, availability, articlesCompletedStart, articlesCompletedEnd): Promise<void> {
    ...
    this.articlesFirmSelectedName = (this.articlesFirmPredefined.length === this.articles.length)
      ...
      : this.articlesFirmPredefined;
    try {
      this.fetchListOfApplicants(jobID, this.pagination, limit, search, this.articlesFirmSelectedName.join(','), gender, ethnicity, nationality, location, qualification, video, availability, articlesCompletedStart, articlesCompletedEnd);
      this._openButton.nativeElement.innerHTML = 'Open more filters';
      this._openFilters.nativeElement.classList.remove('active');
      this._filterFont.nativeElement.classList.remove('active');
      this._filterItem.nativeElement.classList.remove('active');
   ...
   * @param video {number}
   * @param availability {number}
   * @param articlesCompletedStart {number}
   * @param articlesCompletedEnd {number}
   * @return {Promise<void>}
   */
  public async getApplicantsCount(jobID, page, limit, search, articlesFirm, gender, ethnicity, nationality, location, qualification, video, availability, articlesCompletedStart, articlesCompletedEnd): Promise<void> {
    ...
    if (startDate > endDate) {
      ...
      try {
        const response = await this._businessService.getApplicantsCount(1, data);
