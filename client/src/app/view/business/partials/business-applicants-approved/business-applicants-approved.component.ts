...
import { BusinessApplicant, ApplicantsList } from '../../../../../entities/models';
...
@Component({
  selector: 'app-business-applicants-approved',
  templateUrl: './business-applicants-approved.component.html',
  styleUrls: ['./business-applicants-approved.component.scss']
})
export class BusinessApplicantsApprovedComponent implements OnInit {
  ...
  public applicantsApproved = new Array<BusinessApplicant>();

  public listOfJobs: BusinessApplicant[];
  ...
  public async fetchListOfApplicants(jobID: number, page, limit, search, articlesFirm, gender, ethnicity, nationality, location, qualification, video, availability, articlesCompletedStart, articlesCompletedEnd): Promise<void> {
    ...
    if (startDate > endDate) {
      ...
      try {
        const response = await this._businessService.getApplicantsApproved(data);
        ...
        response.items.forEach((item) => {
          ...
          this.applicantsApproved.push(item);
        });

        if (response.pagination.total_count === this.applicantsApproved.length) {
          this.loadMoreCheck = false;
        }
        else if (response.pagination.total_count !== this.applicantsApproved.length) {
  ...
  public async getApplicantsCount(jobID, page, limit, search, articlesFirm, gender, ethnicity, nationality, location, qualification, video, availability, articlesCompletedStart, articlesCompletedEnd): Promise<void> {
    ...
    if (startDate > endDate) {
      ...
      try {
        const response = await this._businessService.getApplicantsCount(3, data);
