...
export class AdminDashboardComponent implements OnInit {
  ...
  public approveCandidateVideoList = Array<CandidateVideoApprove>();
  ...
  public async uploadAdminVideo(userId, index, fileName): Promise<any> {
    ...
    let elem;
    if(!fileName) {
      elem = (<HTMLInputElement>document.getElementById(index + 't'));
    } else {
      elem = (<HTMLInputElement>document.getElementById(fileName));
    }
    const formData = new FormData();
   ...
   * @returns {Promise<void>}
   */
  public async getDashboardData(limit = ''): Promise<void> {
    try {
      ...
      this.approveCandidateList.map(data => {
        data.articlesFirm = data.articlesFirm.replace(/\s/g, '');
      });
      ...
      this.approveCandidateVideoList = dashboardData.newVideos;
      this.approveCandidateVideoList.forEach(() => {
